...
import static mylibs.Data.lots;
import mylibs.Lot;
...
public class LotReport extends javax.swing.JFrame {
    ...
    private void displayLotReport()
    {
        DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        String currentDate = LocalDate.now().format(dateFormatter);

        // Calculate total unique blocks and total lots
        int totalLots = lots.size();
        int totalBlocks = lots.stream().map(Lot::getBlock).collect(Collectors.toSet()).size();

        // Create a formatted string for the report header
        StringBuilder report = new StringBuilder();
        report.append("Lots Report\n\n")
              .append("Total Blocks: \t").append(totalBlocks).append("\n")
              .append("Total Lots: \t").append(totalLots).append("\n\n")
              .append("---------------------------------------------------------------------------------------------------------------------------------\n")
              .append("\t\t\tLots List\n")
              .append("---------------------------------------------------------------------------------------------------------------------------------\n\n");

        // Table Header
        report.append(String.format("%-10s %-10s %-10s %-15s %-15s %-30s\n", 
            "Block", "Lot No", "Size", "Price", "Status", "Location"))
              .append("----------------------------------------------------------------------------------------------------------------------------------\n");

        // Append each lot's details to the report in table row format
        for (Lot lot : lots) {
            report.append(String.format("%-10d %-10d %-10.2f %-14.2f %-16s %-30s\n",
                    lot.getBlock(),
                    lot.getLotNumber(),
                    lot.getSize(),
                    lot.getPrice(),
